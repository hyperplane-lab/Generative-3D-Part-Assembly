'''
    the version without matching loss
    for B-LSTM
...
from data_v2 import PartNetPartDataset
import utils
...
def test(conf):
    data_features = ['part_pcs', 'part_poses', 'part_valids', 'shape_id', 'part_ids', 'contact_points', 'sym']
...
def forward(batch, data_features, network, conf, \
    ...
    gt_part_poses = torch.cat(batch[data_features.index('part_poses')], dim=0).to(conf.device)
    ...
    for repeat_ind in range(repeat_times):
        ...
        pred_part_poses = network(input_part_pcs.float(), input_part_valids.float())     # B x P x P, B x P, B x P x N x 3
    ...
    with torch.no_grad():
        ...
        if is_val and (not conf.no_visu):
            ...
            out_dir = os.path.join(visu_dir, 'test')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str, default='../../data/partnet_dataset/', help='data directory')
    ...
    parser.add_argument('--iter', default = 3, help = 'times to iteration')
    parser.add_argument('--iter_to_test', default = 2, help = 'times to iteration')
    ...
    conf.exp_name = f'exp-{conf.category}-{conf.model_version}-{conf.category}-{conf.exp_suffix}'
