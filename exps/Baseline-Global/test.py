...
def test(conf):
    data_features = ['part_pcs', 'part_poses', 'part_valids', 'shape_id', 'part_ids', 'contact_points', 'sym', 'match_ids']
...
def forward(batch, data_features, network, conf, \
    ...
    for repeat_ind in range(repeat_times):
        ...
        pred_part_poses = network(input_part_pcs.float(), input_part_valids.float(), instance_label)     # B x P x P, B x P, B x P x N x 3
