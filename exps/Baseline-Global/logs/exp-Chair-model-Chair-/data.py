...
class PartNetPartDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for feat in self.data_features:
            ...
            elif feat == "contact_list1":
                cur_num_part = cur_contacts.shape[0]
                out = np.zeros((self.max_num_part,self.max_num_part, 64, 4), dtype=np.float32)
                cur_contact_list1 = cur_contact_list["contact_list1"]
                out[:cur_num_part,:cur_num_part,:,:] = cur_contact_list1
                out = torch.from_numpy(out).float().unsqueeze(0)
                data_feats = data_feats + (out,)

            elif feat == "contact_list2":
                cur_num_part = cur_contacts.shape[0]
                out = np.zeros((self.max_num_part,self.max_num_part, 64, 4), dtype=np.float32)
                cur_contact_list2 = cur_contact_list["contact_list2"]
                cur_contact_list2 = torch.tensor(cur_contact_list2)
                out[:cur_num_part,:cur_num_part,:,:] = cur_contact_list2[:,:,:,:4]
                out = torch.from_numpy(out).float().unsqueeze(0)
                data_feats = data_feats + (out,)
