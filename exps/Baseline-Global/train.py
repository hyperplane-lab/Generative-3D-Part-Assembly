...
def train(conf):
    ...
    data_features = ['part_pcs', 'part_poses', 'part_valids', 'shape_id', 'part_ids', 'match_ids']
...
def forward(batch, data_features, network, conf, \
    ...
    for repeat_ind in range(repeat_times):
        ...
        pred_part_poses = network(input_part_pcs, input_part_valids, instance_label).to(conf.device)     # B x P x (3 + 4)
