...
def test(conf):
    data_features = ['part_pcs', 'part_poses', 'part_valids', 'shape_id', 'part_ids', 'contact_points', 'sym', 'match_ids', 'seq']
...
def forward(batch, data_features, network, conf, \
    ...
    for repeat_ind in range(repeat_times):
        ...
        pred_part_poses = network(seq,input_part_pcs, input_part_valids,
                                  instance_label, gt_part_poses[:, 0, :]).to(conf.device)     # B x P x (3 + 4)
